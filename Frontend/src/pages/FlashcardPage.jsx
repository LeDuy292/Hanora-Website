...
export function FlashcardPage() {
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (!editDeckName.trim()) {
      toast.warning(isEn ? 'Deck name cannot be empty.' : 'Tên bộ Flashcard không được để trống.');
      return;
    }
    setIsSavingEdit(true);
    try {
      await store.updateDeck(editingDeck.id, editDeckName.trim(), editDeckDesc.trim() || null);
      toast.success(isEn ? 'Flashcard deck updated successfully!' : 'Cập nhật bộ Flashcard thành công!');
      setShowEditDeckModal(false);
      loadCustomDecks();
      store.fetchUserFlashcards(null, language);
    } catch (err) {
      toast.error(isEn ? 'Error updating flashcard deck.' : 'Lỗi khi cập nhật bộ Flashcard.');
    } finally {
  ...
  const handleDeleteDeck = (deckId) => {
    toast.confirm(
      isEn ? 'Are you sure you want to delete this flashcard deck? All cards inside will be deleted.' : 'Bạn có chắc chắn muốn xóa bộ Flashcard này? Toàn bộ thẻ trong bộ sẽ bị xóa.',
      async () => {
        try {
          await store.deleteDeck(deckId);
          toast.success(isEn ? 'Flashcard deck deleted successfully!' : 'Xóa bộ Flashcard thành công!');
          loadCustomDecks();
          store.fetchUserFlashcards(null, language);
          ...
          toast.error(isEn ? 'Error deleting flashcard deck.' : 'Lỗi khi xóa bộ Flashcard.');
        }
      },
      isEn ? 'Delete Flashcard Deck' : 'Xóa bộ Flashcard'
    );
  };
  ...
  const handleDuplicateDeck = async (deckId) => {
    try {
      ...
      toast.success(isEn ? 'Deck duplicated successfully!' : 'Sao chép bộ thẻ thành công!');
      loadCustomDecks();
      store.fetchUserFlashcards(null, language);
    } catch (err) {
      toast.error(isEn ? 'Could not duplicate deck.' : 'Không thể sao chép bộ thẻ.');
    }
  };
  ...
  const handleOpenWordsModal = async (deck) => {
    ...
    try {
      const data = await apiRequest(`/flashcard?deckId=${deck.id}&language=${language}`, { auth: true });
      setDeckWords(data || []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleRemoveCard = async (cardId) => {
    toast.confirm(
      isEn ? 'Are you sure you want to remove this word from the deck?' : 'Bạn có chắc chắn muốn loại bỏ từ này khỏi bộ thẻ?',
      async () => {
        try {
          await store.removeCardFromDeck(cardId);
          toast.success(isEn ? 'Word removed from deck successfully!' : 'Đã loại bỏ từ khỏi bộ thẻ thành công!');
          setDeckWords(prev => prev.filter(w => w.id !== cardId));
          loadCustomDecks();
          store.fetchUserFlashcards(null, language);
        } catch (err) {
          toast.error(isEn ? 'Error removing word.' : 'Lỗi khi loại bỏ từ.');
        }
      },
      isEn ? 'Remove Word' : 'Loại bỏ từ'
    );
  };
  ...
  const handleAddWordSubmit = async (e) => {
    e.preventDefault();
    if (!newWordText.trim()) {
      toast.warning(isEn ? 'Please enter a vocabulary word.' : 'Vui lòng nhập từ vựng.');
      return;
    }
    ...
    try {
      ...
      toast.success(isEn ? 'Word added successfully!' : 'Đã thêm từ vựng thành công!');
      setShowAddWordModal(false);
      loadCustomDecks();
      store.fetchUserFlashcards(null, language);
    } catch (err) {
      toast.error(isEn ? 'Error adding word.' : 'Lỗi khi thêm từ.');
    } finally {
  ...
  const handleReviewFeedback = async (know) => {
    ...
    try {
      ...
      if (reviewIndex < reviewCards.length - 1) {
        ...
        toast.success(isEn ? 'Congratulations! You finished your review session.' : 'Chúc mừng! Bạn đã hoàn thành phiên ôn tập.');
        startReviewMode();
        loadDashboardStats();
      }
    } catch (err) {
  ...
  const startWriteMode = async () => {
    try {
      const data = await store.fetchWriteCards(activeDeck?.id === 'all' ? null : activeDeck?.id, 10, language);
      setWriteCards(data || []);
      setWriteIndex(0);
      setUserAnswer('');
  ...
  const handleWriteNext = () => {
    ...
    if (writeIndex < writeCards.length - 1) {
      ...
      toast.success(isEn ? 'Congratulations! You completed writing practice.' : 'Chúc mừng! Bạn đã hoàn thành bài tập viết.');
      startWriteMode();
    }
  };
  ...
  const startMatchMode = async () => {
    try {
      ...
      if (game && !game.error) {
        ...
        toast.warning(game?.error || (isEn ? 'Need at least 4 words to start Match Mode.' : 'Cần ít nhất 4 từ để bắt đầu Match Mode.'));
      }
    } catch (err) {
      console.error(err);
    }
    ...
    if (!selectedTile) {
      ...
      if (selectedTile.cardId === tile.cardId && selectedTile.type !== tile.type) {
        ...
        if (matchedIds.size + 1 === matchGame.cards.length) {
          ...
          toast.success(isEn ? 'Match Mode reward claimed successfully!' : 'Nhận thưởng Match Mode thành công!');
        }
      } else {
        setSelectedTile(tile);
      }
  ...
  useEffect(() => {
    ...
    if (deckIdParam) {
      ...
      if (recentWords.length > 0) {
        setActiveDeck({ id: 'recent', title: isEn ? `⚡ Review ${recentWords.length} recently saved words` : `⚡ Ôn tập ${recentWords.length} từ vừa lưu` });
        setStudyMode('review');
      }
    }
  ...
  const { decks, counts } = useMemo(() => {
    ...
    list.push({ id: 'all', title: isEn ? 'All Vocabulary' : 'Tất cả từ vựng', icon: '📖', count: sourceList.length });
    list.push({ id: 'new', title: isEn ? 'New Words' : 'Từ mới', icon: '🆕', count: newWords.length });
    list.push({ id: 'due', title: isEn ? 'Due for Review (SRS)' : 'Từ cần ôn (SRS)', icon: '⏰', count: dueWords.length });
    list.push({ id: 'hard', title: isEn ? 'Frequently Mistaken' : 'Từ thường trả lời sai', icon: '⚠️', count: hardWords.length });
    list.push({ id: 'known', title: isEn ? 'Mastered Words' : 'Từ đã ghi nhớ', icon: '✅', count: knownWords.length });
  ...
  }, [vocabList, activeDeckCards, activeDeck, isEn]);
      ...
      <div className="px-3 py-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto space-y-5 sm:space-y-8 font-sans">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">
              {isEn ? 'My Flashcards' : 'Flashcard của tôi'}
            </h1>
            <p className="text-slate-500 text-sm mt-1">
              {isEn ? 'Manage and practice personalized vocabulary with Spaced Repetition (SRS)' : 'Quản lý và ôn tập từ vựng cá nhân hóa theo phương pháp SRS'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{isEn ? 'Total Decks' : 'Tổng bộ thẻ'}</span>
            <span className="text-3xl font-black text-slate-800 mt-1">{dashboardStats.totalDecks}</span>
          </div>
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{isEn ? 'Total Words' : 'Tổng số từ'}</span>
            <span className="text-3xl font-black text-slate-800 mt-1">{dashboardStats.totalWords}</span>
          </div>
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{isEn ? 'Mastered' : 'Đã thuộc'}</span>
            <span className="text-3xl font-black text-emerald-600 mt-1">{dashboardStats.masteredWords}</span>
          </div>
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{isEn ? 'Learning' : 'Đang học'}</span>
            <span className="text-3xl font-black text-blue-600 mt-1">{dashboardStats.learningWords}</span>
          </div>
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{isEn ? 'Due Review' : 'Cần ôn tập'}</span>
            <span className="text-3xl font-black text-amber-600 mt-1">{dashboardStats.dueForReview}</span>
          </div>
          <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-col justify-center items-center text-center">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{isEn ? 'Earned XP' : 'Tích lũy XP'}</span>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-sm flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
          <div className="relative w-full md:w-80">
            ...
            <input 
              type="text" 
              placeholder={isEn ? "Search flashcard decks, sources..." : "Tìm kiếm bộ Flashcard, nguồn..."}
              value={searchTerm}
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 w-full md:w-auto">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="learning">{isEn ? 'Learning' : 'Đang học'}</option>
                <option value="not_started">{isEn ? 'Not Started' : 'Chưa học'}</option>
                <option value="completed">{isEn ? 'Completed' : 'Hoàn thành'}</option>
                <option value="recently_created">{isEn ? 'Recently Created' : 'Tạo gần đây'}</option>
                <option value="recently_studied">{isEn ? 'Recently Studied' : 'Học gần đây'}</option>
              </select>
            </div>
            ...
            >
              <option value="created_desc">{isEn ? 'Date Created (Newest)' : 'Ngày tạo (Mới nhất)'}</option>
              <option value="created_asc">{isEn ? 'Date Created (Oldest)' : 'Ngày tạo (Cũ nhất)'}</option>
              <option value="name_asc">{isEn ? 'Name A-Z' : 'Tên A-Z'}</option>
              <option value="name_desc">{isEn ? 'Name Z-A' : 'Tên Z-A'}</option>
              <option value="progress_desc">{isEn ? 'Highest Progress' : 'Tiến độ cao nhất'}</option>
              <option value="words_desc">{isEn ? 'Most Words' : 'Số từ nhiều nhất'}</option>
            </select>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4">
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            ...
            <span>{isEn ? 'My Flashcard Decks' : 'Bộ thẻ của tôi'} ({customDecks.length})</span>
          </h2>
          
          {isLoadingDecks ? (
            <div className="text-center py-20 font-bold text-slate-450 animate-pulse text-sm">
              {isEn ? 'Loading decks list...' : 'Đang tải danh sách bộ thẻ...'}
            </div>
          ) : customDecks.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
              <BookMarked className="w-12 h-12 text-slate-200 mx-auto mb-3" />
              <h3 className="text-base font-bold text-slate-700">{isEn ? 'No decks found' : 'Không tìm thấy bộ thẻ nào'}</h3>
              <p className="text-slate-400 text-xs mt-1">
                {isEn ? 'Try searching with different keywords or create a new deck from your vocabulary notebook.' : 'Hãy thử tìm kiếm từ khóa khác hoặc tạo bộ mới từ sổ tay từ vựng.'}
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="space-y-3">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-1">
                        <button 
                          ...
                          title={isEn ? "Duplicate deck" : "Sao chép bộ thẻ"}
                        >
                        ...
                        <button 
                          ...
                          title={isEn ? "Add new word" : "Thêm từ mới"}
                        >
                        ...
                        <button 
                          ...
                          className="text-slate-400 hover:text-blue-600 p-1.5 hover:bg-slate-50 rounded-lg transition"
                          title={isEn ? "Edit deck info" : "Sửa thông tin"}
                        >
                        ...
                        <button 
                          ...
                          title={isEn ? "Word list" : "Danh sách từ"}
                        >
                    ...
                    <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed">
                      {deck.description || (isEn ? "No description for this deck." : "Không có mô tả cho bộ thẻ này.")}
                    </p>
                    
                    <div className="space-y-1.5 pt-2 text-xs text-slate-500 font-semibold">
                      <div className="flex justify-between items-center text-[11px]">
                        <span>{isEn ? 'Source: ' : 'Nguồn: '}<span className="text-slate-700">{deck.source || (isEn ? 'Notebook' : 'Sổ tay')}</span></span>
                        <span className="text-slate-700 font-bold">{deck.learnedWords || 0} / {deck.totalWords} {isEn ? 'words learned' : 'từ đã học'}</span>
                      </div>
                      ...
                      <div className="flex justify-between text-[9px] text-slate-400 pt-1 font-bold">
                        <span>{isEn ? 'Created: ' : 'Tạo: '}{new Date(deck.createdAt).toLocaleDateString(isEn ? 'en-US' : 'vi-VN')}</span>
                        <span>{isEn ? 'Studied: ' : 'Học: '}{deck.lastStudiedAt ? new Date(deck.lastStudiedAt).toLocaleDateString(isEn ? 'en-US' : 'vi-VN') : (isEn ? 'Not studied' : 'Chưa học')}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-100 pt-4 flex items-center justify-between gap-2.5">
                    <button 
                      ...
                      title={isEn ? "Quick flashcards view" : "Đọc lướt và xem thẻ nhanh"}
                    >
                      <Eye className="w-3.5 h-3.5 text-slate-400 group-hover/btn:text-blue-500" />
                      <span>{isEn ? 'Flashcard' : 'Đọc thẻ'}</span>
                    </button>
                    ...
                    <button 
                      ...
                      title={isEn ? "Study with SRS and earn XP" : "Chế độ học từ vựng SRS và tích lũy XP"}
                    >
                      <BookOpen className="w-3.5 h-3.5" />
                      <span>{isEn ? 'Study Words' : 'Học từ'}</span>
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 flex flex-col space-y-4 text-slate-700">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <h3 className="text-base font-extrabold text-slate-800">{isEn ? 'Edit Flashcard Deck' : 'Sửa bộ Flashcard'}</h3>
                <button 
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-450 uppercase tracking-wider">{isEn ? 'Deck Description' : 'Mô tả bộ thẻ'}</label>
                  <textarea
                    value={editDeckDesc}
                    onChange={(e) => setEditDeckDesc(e.target.value)}
                    rows={3}
                    placeholder={isEn ? "Describe learning goals for this deck..." : "Mô tả mục tiêu học tập của bộ này..."}
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    {isSavingEdit ? (isEn ? 'Saving...' : 'Đang lưu...') : (isEn ? 'Save' : 'Lưu lại')}
                  </button>
                </div>
              </form>
            </div>
        ...
        {showCardsListModal && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-100 flex flex-col space-y-4 text-slate-700">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <div>
                  <h3 className="text-base font-extrabold text-slate-800">{isEn ? 'Words in Deck' : 'Từ vựng trong bộ'}</h3>
                  <p className="text-[11px] text-slate-400 font-semibold">{selectedDeckForWords?.name} ({deckWords.length} {isEn ? 'words' : 'từ'})</p>
                </div>
                <button 
              ...
              <div className="max-h-96 overflow-y-auto space-y-2 pr-1 font-sans">
                {isLoadingDeckWords ? (
                  <div className="text-center py-8 text-xs font-bold text-slate-400 animate-pulse">{isEn ? 'Loading words list...' : 'Đang tải danh sách từ...'}</div>
                ) : deckWords.length === 0 ? (
                  <div className="text-center py-8 text-xs font-bold text-slate-400">{isEn ? 'No words in this deck.' : 'Bộ thẻ chưa có từ vựng nào.'}</div>
                ) : (
                  deckWords.map((card) => (
        ...
        {showAddWordModal && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 flex flex-col space-y-4 text-slate-700 font-sans">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <h3 className="text-base font-extrabold text-slate-800">{isEn ? 'Add Word to Deck' : 'Thêm từ vào bộ Flashcard'}</h3>
                <button 
              ...
              <form onSubmit={handleAddWordSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-450 uppercase tracking-wider">{isEn ? 'Chinese Word' : 'Từ vựng tiếng Trung'}</label>
                  <input
                    type="text"
                    value={newWordText}
                    onChange={(e) => setNewWordText(e.target.value)}
                    placeholder={isEn ? "e.g.: 学习, 苹果, 谢谢..." : "Ví dụ: 学习, 苹果, 谢谢..."}
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-bold text-slate-700 focus:outline-none focus:border-blue-500"
                    required
                  />
                  <p className="text-[10px] text-slate-400 mt-1">
                    {isEn ? 'System will automatically look up Pinyin, English/Vietnamese definitions and example sentences with AI.' : 'Hệ thống sẽ tự động tra cứu Pinyin, nghĩa tiếng Việt và các câu ví dụ minh họa bằng AI.'}
                  </p>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition"
                  >
                    {isAddingWord ? (isEn ? 'Adding...' : 'Đang thêm...') : (isEn ? 'Add Word' : 'Thêm từ')}
                  </button>
                </div>
              </form>
            </div>
    ...
    <div className="flashcard-container page-transition" style={{ padding: studyMode === 'learn' ? '1rem 2rem' : '2rem' }}>
        ...
        <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-600 text-white rounded-3xl p-5 shadow-lg relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-3 shrink-0 relative z-10">
            <div className="text-center px-2">
              <span className="text-[10px] font-bold text-white/70 uppercase block">{isEn ? 'Card Count' : 'Số lượng thẻ'}</span>
              <span className="text-lg font-black text-white">{activeList.length} {isEn ? 'Cards' : 'Thẻ'}</span>
            </div>
            <div className="h-8 w-px bg-white/20"></div>
            <div className="text-center px-2">
              <span className="text-[10px] font-bold text-white/70 uppercase block">{isEn ? 'Estimated' : 'Dự kiến phiên'}</span>
              <span className="text-lg font-black text-white">~{Math.max(3, Math.ceil((activeList.length * 25) / 60))} {isEn ? 'min' : 'phút'}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1">
        ...
        >
          ...
          <span>{isEn ? 'Cards' : 'Flashcard'}</span>
        </button>
        <button 
        ...
        >
          ...
          <span>{isEn ? 'Learn' : 'Học (Learn)'}</span>
        </button>
        <button 
        ...
        >
          ...
          <span>{isEn ? 'Quiz / Test' : 'Kiểm tra (Test)'}</span>
        </button>
        <button 
        ...
        >
          ...
          <span>{isEn ? 'Match Game' : 'Ghép từ (Match)'}</span>
        </button>
        <button 
        ...
        >
          ...
          <span>{isEn ? 'SRS Review' : 'Ôn tập SRS'}</span>
        </button>
      </div>
      ...
      {studyMode === 'learn' && !learnComplete && (
        <div className="bg-white border border-slate-100/80 p-3 px-5 rounded-2xl shadow-xs space-y-2">
          <div className="flex justify-between items-baseline text-xs font-bold text-slate-500">
            <span className="text-slate-700">{isEn ? 'Study Words' : 'Học từ'}</span>
        ...
        <div className="space-y-4 w-full max-w-[92vw] md:max-w-[92%] mx-auto pt-2 animate-fade-in font-sans">
          ...
          {learnComplete ? (
            <div className="bg-white border border-slate-100 p-8 sm:p-10 rounded-3xl shadow-sm text-center space-y-6 animate-pop">
              ...
              <div>
                <h2 className="text-3xl font-black text-slate-800">{isEn ? 'Session Completed!' : 'Hoàn thành phiên học!'}</h2>
                <p className="text-slate-500 text-sm mt-1">{isEn ? 'Congratulations on completing your vocabulary lesson.' : 'Chúc mừng bạn đã hoàn thành bài học từ vựng.'}</p>
              </div>

              <div className="grid grid-cols-3 gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-100 max-w-md mx-auto">
                <div className="text-center border-r border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">{isEn ? 'Correct' : 'Chính xác'}</span>
                  <span className="text-2xl font-black text-emerald-600 block mt-1">{learnScore}</span>
                </div>
                <div className="text-center border-r border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 uppercase block tracking-widest">{isEn ? 'Incorrect' : 'Sai'}</span>
                  <span className="text-2xl font-black text-rose-600 block mt-1">{learnIncorrectCount}</span>
                </div>
                <div className="text-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase block tracking-widest">{isEn ? 'Accuracy' : 'Độ chính xác'}</span>
              ...
              {learnFailedCards.length > 0 && (
                <div className="text-left space-y-3 max-w-md mx-auto">
                  <h4 className="text-sm font-bold text-slate-700">{isEn ? 'Words to review:' : 'Các từ cần học lại:'}</h4>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2 max-w-md mx-auto">
                {learnFailedCards.length > 0 && (
                  ...
                  >
                    {isEn ? 'Review Missed Words' : 'Học lại từ chưa nhớ'}
                  </button>
                )}
                <button
                ...
                >
                  {isEn ? 'Restart Session' : 'Học lại từ đầu'}
                </button>
                <button
                  onClick={() => setStudyMode('flashcard')}
                ...
                >
                  {isEn ? 'Back to Decks' : 'Quay về bộ thẻ'}
                </button>
              </div>
            </div>
          ) : !learnQuestion ? (
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm w-full">
              ...
              <p className="text-slate-500 text-xs mt-3 font-semibold">{isEn ? 'Loading question...' : 'Đang tải câu hỏi...'}</p>
            </div>
          ) : (
            <div className={`bg-white border border-slate-100 rounded-[2rem] p-4 sm:p-6 shadow-md space-y-5 w-full transition-all duration-300 ${
              ...
              <div className="text-center space-y-2 py-1">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 border border-slate-200 px-3 py-1 rounded-full inline-block">
                  {learnQuestion.type === 'choice_meaning' ? (isEn ? 'Choose correct meaning' : 'Chọn nghĩa đúng') : 
                   learnQuestion.type === 'choice_word' ? (isEn ? 'Find the Chinese word' : 'Tìm từ tiếng Trung') : 
                   learnQuestion.type === 'choice_pinyin' ? (isEn ? 'Select correct pinyin' : 'Chọn phiên âm đúng') :
                   learnQuestion.type === 'audio_choice' ? (isEn ? 'Listen and select correct word' : 'Nghe phát âm và chọn từ đúng') :
                   learnQuestion.type === 'fill_blank' ? (isEn ? 'Fill in the blank' : 'Điền vào chỗ trống') : 
                   (isEn ? 'Type English meaning' : 'Nhập nghĩa tiếng Việt')}
                </span>
                ...
                {learnQuestion.type === 'audio_choice' ? (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-xs text-slate-450 font-bold">{isEn ? 'Click speaker to listen' : 'Bấm loa để nghe phát âm'}</span>
                  </div>
                ) : (
              ...
              {!learnAnswered ? (
                ...
                learnQuestion.type === 'input_pinyin' ? (
                  ...
                  >
                    <input 
                      type="text" 
                      placeholder={
                        learnQuestion.type === 'input_pinyin' ? (isEn ? "Type pinyin (e.g. zi xin)..." : "Nhập phiên âm pinyin (ví dụ: zi xin)...") :
                        learnQuestion.type === 'input_word' ? (isEn ? "Type Chinese word..." : "Nhập từ tiếng Trung...") :
                        (isEn ? "Type English meaning..." : "Nhập nghĩa tiếng Việt...")
                      }
                ...
                <div className="space-y-4 text-left border-t border-slate-100 pt-4 animate-in w-full">
                    ...
                    <div className="flex items-center gap-2 font-black text-sm uppercase tracking-wide">
                      {learnAnswered.isCorrect ? (
                        <>
                          ...
                          <span>{isEn ? 'CORRECT (+10 XP)' : 'CHÍNH XÁC (+10 XP)'}</span>
                        </>
                      ) : (
                        <>
                          <X className="w-5 h-5 text-rose-600" />
                          <span>{isEn ? 'INCORRECT' : 'CHƯA CHÍNH XÁC'}</span>
                        </>
                      )}
                    </div>
                    {!learnAnswered.isCorrect && (
                      <p className="text-xs mt-2 font-bold text-slate-755">
                        {isEn ? 'Correct answer: ' : 'Đáp án đúng: '}<span className="font-black text-rose-650 bg-rose-100/50 px-2 py-0.5 rounded-lg underline">{learnAnswered.correctOption}</span>
                      </p>
                    )}
                  </div>
                  ...
                  <div className="bg-slate-50/50 border border-slate-150 rounded-2xl p-4 sm:p-5 space-y-3">
                    <div className="flex justify-between items-center border-b border-slate-200/30 pb-2">
                      ...
                      <button 
                        ...
                        title={isEn ? "Listen to pronunciation" : "Nghe phát âm"}
                      >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-650">
                      ...
                      <div className="space-y-2">
                        {learnQuestion.exampleZh ? (
                          <div>
                            <span className="text-[10px] font-black text-slate-400 block uppercase tracking-wider mb-0.5">
                              {isEn ? 'Example Sentence' : 'Ví dụ minh họa'}
                            </span>
                          ...
                          <div className="text-slate-400 italic text-center py-4">
                            {isEn ? 'No example sentence available for this word.' : 'Chưa có câu ví dụ cho từ này.'}
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex justify-center sm:justify-end pt-1">
                    ...
                    >
                      {isEn ? 'Continue → (Space / Enter)' : 'Tiếp tục → (Space / Enter)'}
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex justify-between items-center bg-slate-50 p-4 rounded-3xl border border-slate-100 mb-4">
              <div className="flex gap-2">
                ...
                >
                  ...
                  <span>{isEn ? `Pinyin: ${showPinyin ? 'On' : 'Off'}` : `Pinyin: ${showPinyin ? 'Bật' : 'Tắt'}`}</span>
                </button>
              </div>
            ...
            <div className="progress-section">
              <div className="progress-info">
                <div className="progress-text">{isEn ? `Studying card ${currentIndex + 1} / ${activeList.length}` : `Đang học thẻ ${currentIndex + 1} / ${activeList.length}`}</div>
                <div className="time-remaining">
                  <Clock className="w-3.5 h-3.5" />
                  {isEn 
                    ? `Studied ${user?.todayMinutes || 0} / ${user?.targetDailyMinutes || 20} min today` 
            ...
            <div className="flashcard-area-wrapper">
              {isSessionComplete ? (
                <div className="session-complete-view animate-in">
                  ...
                  <h2 className="session-title">{isEn ? 'Awesome!' : 'Tuyệt vời!'}</h2>
                  <p className="session-subtitle">
                    {isEn ? 'You have completed the deck ' : 'Bạn đã hoàn thành bộ thẻ '}
                  ...
                  <div className="session-stats-summary">
                    <div className="summary-item">
                      ...
                      <span className="summary-label">{isEn ? 'Cards Studied' : 'Thẻ đã học'}</span>
                    </div>
                    <div className="summary-item">
                      <span className="summary-value text-emerald-500">{sessionStats.easyCount}</span>
                      <span className="summary-label">{isEn ? 'Remembered' : 'Ghi nhớ tốt'}</span>
                    </div>
                    <div className="summary-item">
                      <span className="summary-value text-rose-500">{sessionStats.hardCount}</span>
                      <span className="summary-label">{isEn ? 'Needs Review' : 'Cần xem lại'}</span>
                    </div>
                  </div>
                  ...
                  <div className="session-actions">
                    ...
                    >
                      {isEn ? 'Study Another Deck' : 'Học tiếp bộ khác'}
                    </Button>
                  </div>
                </div>
              ) : currentWord ? (
                <>
                  ...
                  <div className="nav-bar mt-4">
                    <div className="nav-group">
                      <button className={`nav-btn ${isShuffled ? 'active' : ''}`} onClick={handleToggleShuffle} title={isEn ? "Shuffle cards" : "Trộn thứ tự thẻ"}>
                        <Shuffle className="w-4 h-4" />
                        {isEn ? 'Shuffle' : 'Trộn'}
                      </button>
                      <button className={`nav-btn ${isPlaying ? 'active' : ''}`} onClick={() => setIsPlaying(!isPlaying)} title={isEn ? "Auto flip and advance" : "Tự động lật và chuyển thẻ"}>
                    ...
                    <div className="hidden sm:flex items-center text-xs font-semibold text-slate-400">
                      <span>{isEn ? 'Tip: Use keys ' : 'Mẹo: Dùng phím '}<kbd className="px-1.5 py-0.5 text-[10px] font-mono text-slate-600 bg-slate-100 border border-slate-200 rounded">{isEn ? 'Left' : 'Trái'}</kbd> <kbd className="px-1.5 py-0.5 text-[10px] font-mono text-slate-600 bg-slate-100 border border-slate-200 rounded">{isEn ? 'Right' : 'Phải'}</kbd> {isEn ? 'to switch cards' : 'để chuyển thẻ'}</span>
                    </div>
                ...
                <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
                  ...
                  <h3 className="text-xl font-bold text-slate-800">{isEn ? 'Empty Deck' : 'Bộ từ vựng trống'}</h3>
                  <p className="text-slate-500">{isEn ? 'Please choose another deck or return to library.' : 'Mời bạn chọn bộ khác hoặc quay lại thư viện.'}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-1.5">
              ...
              <span>{isEn ? 'Spaced Repetition Review (SRS)' : 'Ôn tập theo lịch lặp lại ngắt quãng (SRS)'}</span>
            </h3>
            <span className="text-xs font-bold text-slate-500">{isEn ? 'Card' : 'Thẻ'} {reviewIndex + 1} / {reviewCards.length}</span>
          </div>

          {reviewCards.length === 0 ? (
            <div className="text-center py-16 text-slate-400 font-bold text-xs">{isEn ? 'No cards due for review in this deck!' : 'Không có thẻ nào đến hạn ôn tập trong bộ này!'}</div>
          ) : (
            <div className="space-y-6">
              ...
              >
                <span className="absolute top-4 left-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">{isEn ? 'Click to flip' : 'Bấm để lật'}</span>
              ...
              <div id="srs-rating" className="flex gap-4">
                ...
                >
                  {isEn ? 'Hard (Review soon)' : 'Chưa thuộc (Lặp lại sớm)'}
                </button>
                <button 
                ...
                >
                  {isEn ? 'Good (Earn +5 XP)' : 'Đã thuộc (Nhận XP +5)'}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-1.5">
              ...
              <span>{isEn ? 'Write Mode - Type English meaning' : 'Write Mode - Nhập nghĩa tiếng Việt'}</span>
            </h3>
            <span className="text-xs font-bold text-slate-500">{isEn ? 'Word' : 'Từ'} {writeIndex + 1} / {writeCards.length}</span>
          </div>

          {writeCards.length === 0 ? (
            <div className="text-center py-16 text-slate-400 font-bold text-xs">{isEn ? 'No cards to practice writing in this deck!' : 'Không có thẻ nào để luyện viết trong bộ này!'}</div>
          ) : (
            <div className="space-y-6">
              ...
              {!writeFeedback ? (
                <form onSubmit={handleWriteSubmit} className="space-y-4">
                  <input 
                    type="text" 
                    placeholder={isEn ? "Enter the English meaning of this word..." : "Nhập nghĩa tiếng Việt của từ này..."}
                    value={userAnswer}
                  ...
                  >
                    {isSubmitWriteLoading ? (isEn ? 'Checking...' : 'Đang kiểm tra...') : (isEn ? 'Submit Answer' : 'Gửi đáp án')}
                  </button>
                </form>
              ) : (
                <div className="space-y-4 animate-in">
                  <div className={`p-5 rounded-2xl border text-center ${
                    ...
                    <h4 className="text-sm font-extrabold">{writeFeedback.isCorrect ? (isEn ? '✓ CORRECT (+10 XP)' : '✓ CHÍNH XÁC (+10 XP)') : (isEn ? '✗ INCORRECT (+5 XP)' : '✗ CHƯA ĐÚNG (+5 XP)')}</h4>
                    <p className="text-xs mt-1.5">{isEn ? 'Correct answer: ' : 'Đáp án đúng: '}<span className="font-extrabold">{writeFeedback.correctAnswer}</span></p>
                  </div>
                  <button 
        ...
        <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-base font-extrabold text-slate-800 flex items-center gap-1.5">
              ...
              <span>{isEn ? 'Match Game - Pair matching' : 'Match Game - Trò chơi ghép cặp'}</span>
            </h3>
            <span className="text-xs font-bold text-slate-500">{isEn ? 'Matched: ' : 'Đã ghép: '}{matchedIds.size} / {matchGame?.cards?.length || 0} {isEn ? 'pairs' : 'cặp'}</span>
          </div>
          ...
          {matchGameComplete ? (
            <div className="text-center py-12 space-y-4 animate-in">
              ...
              <h3 className="text-lg font-black text-slate-800">{isEn ? 'Game Completed!' : 'Hoàn thành màn chơi!'}</h3>
              <p className="text-slate-500 text-xs">{isEn ? 'You matched all cards successfully and earned bonus XP.' : 'Bạn đã ghép thành công tất cả các thẻ và nhận thưởng XP thành công.'}</p>
              <button 
