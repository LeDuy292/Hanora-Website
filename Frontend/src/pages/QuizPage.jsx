...
const getQuestionTypes = (isEn) => [
  { id: 'multiple_choice_meaning', label: isEn ? 'Choose Meaning' : 'Chọn nghĩa tiếng Việt' },
  { id: 'multiple_choice_word', label: isEn ? 'Choose Chinese Word' : 'Chọn từ tiếng Trung' },
  { id: 'pinyin_match', label: isEn ? 'Match Pinyin' : 'Chọn Pinyin' },
  { id: 'fill_in_blank', label: isEn ? 'Fill in Blank' : 'Điền từ còn thiếu' },
  { id: 'example_match', label: isEn ? 'Match Example Sentence' : 'Chọn câu ví dụ phù hợp' },
  { id: 'context', label: isEn ? 'Context Question' : 'Câu hỏi ngữ cảnh' }
];

const getDifficulties = (isEn) => [
  { id: 'easy', label: isEn ? 'Easy' : 'Dễ' },
  { id: 'medium', label: isEn ? 'Medium' : 'Trung bình' },
  { id: 'hard', label: isEn ? 'Hard' : 'Khó' },
  { id: 'adaptive', label: isEn ? 'Adaptive (AI)' : 'Adaptive (AI quyết định)' }
];
...
export function QuizPage() {
  ...
  const questionTypes = useMemo(() => getQuestionTypes(isEn), [isEn]);
  const difficulties = useMemo(() => getDifficulties(isEn), [isEn]);

  const prettyType = (typeKey) =>
    questionTypes.find(q => q.id === typeKey)?.label ||
    String(typeKey || '').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  const {
  ...
  if (quizLoading && currentStep === 'intro') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50">
        ...
        <p className="text-xl font-bold text-slate-800">{isEn ? 'AI is generating a customized quiz for you...' : 'AI đang tạo đề thi phù hợp với bạn...'}</p>
        <p className="text-slate-500 mt-2">{isEn ? 'This may take a few seconds.' : 'Việc này có thể mất vài giây.'}</p>
      </div>
    );
  }
      ...
      <div className="quiz-page-wrapper flex flex-col items-center py-6 px-4 w-full justify-center min-h-[calc(100vh-80px)]">
        <div className="w-full bg-white p-6 sm:p-10 rounded-[3rem] shadow-2xl shadow-blue-100/50 border border-slate-100/80 animate-in">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-slate-500 max-w-lg mx-auto">{isEn ? 'Personalized test generated by AI from your flashcard decks.' : 'Bài kiểm tra cá nhân hóa do AI tạo từ bộ Flashcard của bạn.'}</p>
            ...
            <div className="flex justify-center mt-3">
              ...
              >
                <History className="w-3.5 h-3.5" /> {t('quiz.viewHistory')}
              </button>
            </div>
          </div>
          ...
          {resumable && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-4 text-left">
              ...
              <div className="flex-1">
                <div className="font-black text-amber-800 text-sm">{isEn ? 'You have an in-progress quiz session' : 'Bạn có một bài thi đang làm dở'}</div>
                <div className="text-xs text-amber-700">{resumable.quizQuestions.length} {isEn ? 'questions' : 'câu'} • {resumable.difficulty}</div>
              </div>
              <Button variant="primary" className="rounded-xl px-4 py-1.5 text-xs shadow-sm" onClick={handleResume}>{isEn ? 'Resume' : 'Tiếp tục'}</Button>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 text-left">
            ...
            <div className="space-y-6 flex flex-col justify-between">
              ...
              <div className="quiz-selection-box bg-slate-50 p-7 sm:p-8 rounded-[2rem] border border-slate-100 flex-1 flex flex-col justify-between">
                <div>
                  <h4 className="font-black text-slate-800 mb-4 text-base sm:text-lg tracking-tight">{t('quiz.questionCount')}</h4>
                  ...
                  {showCustomInput && (
                    <input 
                      ...
                      placeholder={isEn ? "Enter number..." : "Nhập số..."} 
                      value={targetCount} 
                ...
                <div className="flex items-center gap-2 text-slate-400 font-semibold text-xs mt-4 pt-4 border-t border-slate-200/50">
                  ...
                  <span>
                    📖 {isEn ? 'You currently have' : 'Bạn đang có'} <strong className="text-slate-800">{searchParams.get('max') ? searchParams.get('max') : vocabList.length}</strong> {isEn ? 'words' : 'từ vựng'} {searchParams.get('deckId') ? (isEn ? 'in this deck' : 'trong bộ thẻ này') : ''}
                  </span>
                </div>
              </div>
              ...
              <div className="quiz-selection-box bg-slate-50 p-7 sm:p-8 rounded-[2rem] border border-slate-100 flex-1 flex flex-col justify-between">
                <div>
                  <h4 className="font-black text-slate-800 mb-4 text-base sm:text-lg tracking-tight">{t('quiz.selectDifficulty')}</h4>
                  ...
                  {difficulty === 'adaptive' && (
                    <p className="text-[10px] text-blue-600 font-bold italic mt-3 animate-fade-in">
                      {isEn ? '* Adaptive: AI automatically tunes difficulty based on your performance.' : '* Adaptive: AI tự động điều chỉnh độ khó dựa trên khả năng của bạn.'}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="quiz-selection-box bg-slate-50 p-7 sm:p-8 rounded-[2rem] border border-slate-100 text-left flex flex-col justify-between">
              <div>
                <h4 className="font-black text-slate-800 mb-4 text-base sm:text-lg tracking-tight">{t('quiz.selectTypes')}</h4>
                ...
                <label className={`flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer mb-4 transition-all ${aiMixed ? 'border-blue-600 bg-blue-50/50 shadow-sm' : 'border-slate-150 bg-white hover:border-blue-200'}`}>
                  ...
                  <div className="flex flex-col">
                    <span className="font-black text-slate-755 text-sm">✨ {isEn ? 'AI Mixed Mode' : 'AI Mixed Mode'}</span>
                    <span className="text-slate-400 font-medium text-[11px] mt-0.5">{isEn ? 'AI automatically selects the most suitable question formats' : 'AI tự chọn dạng câu hỏi phù hợp nhất'}</span>
                  </div>
                </label>
          ...
          <div className="flex justify-center w-full mt-4">
            ...
            >
              <Sparkles className="w-5 h-5 mr-2 animate-pulse" /> {t('quiz.startQuiz')}
            </Button>
          </div>
        </div>
      </div>
        ...
        <div className="quiz-card animate-in w-full bg-white rounded-[2rem] shadow-xl overflow-hidden border border-slate-100/50">
          ...
          <div className="p-4 sm:p-6 border-b border-slate-100 bg-slate-50/50">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h2 className="text-xl sm:text-2xl font-black text-slate-800">{isEn ? 'Practice Session' : 'Bài Luyện Tập'}</h2>
                <div className="flex gap-2 mt-1.5 flex-wrap">
                  ...
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider bg-white px-2.5 py-0.5 rounded-full border border-slate-150">{total} {isEn ? 'questions' : 'câu'}</span>
          ...
          <div className="p-4 sm:p-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center gap-3 flex-wrap">
            <div className="flex gap-2">
              <Button variant="outline" onClick={handlePrevious} disabled={currentIndex === 0} className="h-11 sm:h-12 rounded-xl px-5 border-slate-200 text-xs font-bold uppercase tracking-wider">
                <ChevronLeft className="mr-1 w-4 h-4" /> {isEn ? 'Back' : 'Trước'}
              </Button>
              <Button variant="outline" onClick={skipQuestion} className="h-11 sm:h-12 rounded-xl px-5 border-slate-200 text-xs font-bold uppercase tracking-wider">
                {isEn ? 'Skip' : 'Bỏ qua'}
              </Button>
            </div>
            <div className="flex gap-2">
      ...
      <div className="quiz-page-wrapper flex flex-col items-center justify-center py-6 px-4 w-full min-h-[calc(100vh-80px)]">
        <div className="w-full bg-white rounded-[3rem] shadow-2xl p-10 md:p-12 animate-in border border-slate-50">
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-black text-slate-900 mb-2">{t('quiz.completedTitle')}</h1>
            <p className="text-base text-slate-500">{t('quiz.completedSubtitle')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-center mb-8 bg-slate-50/50 p-6 rounded-[2.5rem] border border-slate-100/50">
            <div className="flex flex-col items-center justify-center border-b lg:border-b-0 lg:border-r border-slate-100 pb-4 lg:pb-0 lg:pr-6">
              <div className="w-36 h-36 rounded-full border-[8px] border-slate-100 flex flex-col items-center justify-center relative bg-white shadow-sm">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{t('practiceHistory.points')}</span>
              </div>
            </div>
            <div className="lg:col-span-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="stat-card p-4 bg-emerald-50 rounded-2xl border border-emerald-100/70 text-center shadow-xs">
                ...
                <div className="text-emerald-700 uppercase tracking-wider font-extrabold text-[9px]">{isEn ? 'Correct' : 'Đúng'}</div>
              </div>
              <div className="stat-card p-4 bg-rose-50 rounded-2xl border border-rose-100/70 text-center shadow-xs">
                ...
                <div className="text-rose-750 uppercase tracking-wider font-extrabold text-[9px]">{isEn ? 'Wrong' : 'Sai'}</div>
              </div>
              <div className="stat-card p-4 bg-blue-50 rounded-2xl border border-blue-100/70 text-center shadow-xs">
                ...
                <div className="text-blue-750 uppercase tracking-wider font-extrabold text-[9px]">{t('quiz.accuracy')}</div>
              </div>
              <div className="stat-card p-4 bg-indigo-50 rounded-2xl border border-indigo-100/70 text-center shadow-xs">
                ...
                <div className="text-indigo-750 uppercase tracking-wider font-extrabold text-[9px]">{isEn ? 'Time' : 'Thời gian'}</div>
              </div>
              <div className="stat-card p-4 bg-amber-50 rounded-2xl border border-amber-100/70 text-center shadow-xs">
                ...
                <div className="text-amber-750 uppercase tracking-wider font-extrabold text-[9px]">{t('practiceHistory.xp')}</div>
              </div>
              <div className="stat-card p-4 bg-slate-900 rounded-2xl text-center flex flex-col items-center justify-center shadow-xs">
                <div className="text-white font-black text-lg mb-0.5">{session.generator === 'ai' ? 'AI' : (isEn ? 'Standard' : 'Tiêu chuẩn')}</div>
                <div className="text-slate-400 uppercase tracking-wider font-extrabold text-[9px]">{isEn ? 'Mode' : 'Chế độ'}</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="bg-slate-50 p-6 sm:p-7 rounded-[2rem] border border-slate-100">
              <h3 className="font-black text-lg mb-4 flex items-center gap-2 text-slate-800"><Target className="w-5 h-5 text-blue-600" /> {isEn ? 'Skills Radar' : 'Kỹ năng'}</h3>
            ...
            <div className="bg-blue-600 p-6 sm:p-7 rounded-[2rem] text-white flex flex-col shadow-lg shadow-blue-100/50">
              ...
              <p className="text-sm font-medium leading-relaxed text-blue-50 whitespace-pre-line flex-1">
                {session.aiFeedback || (isEn ? "Great job completing the quiz! Keep up regular practice to master more words." : "Tuyệt vời, bạn đã hoàn thành bài thi! Hãy tiếp tục luyện tập đều đặn nhé.")}
              </p>
              {weakWords.length > 0 && (
                <div className="mt-4 pt-4 border-t border-white/20">
                  <h4 className="font-black mb-2 uppercase tracking-widest text-[10px] text-blue-200">{isEn ? 'Words to review' : 'Từ cần ôn lại'}</h4>
                  ...
                  <Button variant="secondary" className="rounded-xl bg-white text-blue-700 hover:bg-blue-50 font-bold text-xs px-3 py-1.5" onClick={reviewWeakWords}>
                    <RotateCcw className="w-3.5 h-3.5 mr-1.5" /> {isEn ? 'Review Weak Words' : 'Ôn lại từ yếu'}
                  </Button>
                </div>
              )}
            </div>
          ...
          {/* Per-question review */}
          <div className="mb-10">
            <h3 className="font-black text-2xl mb-6 flex items-center gap-3"><BookOpen className="w-6 h-6 text-blue-600" /> {t('quiz.reviewAnswers')}</h3>
                ...
                return (
                  <div key={q.id} className={`p-6 rounded-3xl border-2 ${correct ? 'border-emerald-100 bg-emerald-50/40' : 'border-rose-100 bg-rose-50/40'}`}>
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{isEn ? `Question ${i + 1}` : `Câu ${i + 1}`} • {prettyType(q.questionType)}</div>
                    ...
                    <div className="pl-11 flex flex-wrap gap-2 mb-3">
                      ...
                      {!q.userAnswer && <span className="px-3 py-1.5 rounded-lg text-sm font-bold bg-slate-200 text-slate-500">{isEn ? 'Unanswered' : 'Chưa trả lời'}</span>}
                    </div>
                    {(q.aiExplanation || q.explanation) && (
                      <div className="pl-11">
                        <div className="bg-white/70 rounded-2xl p-4 border border-slate-100 text-slate-600 text-sm leading-relaxed">
                          <span className="font-black text-blue-600">{isEn ? 'Explanation: ' : 'Giải thích: '}</span>
                          {q.aiExplanation || q.explanation}
                        </div>
                      </div>
                    )}
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Button variant="outline" className="flex-1 h-16 rounded-[1.5rem] text-xl font-black border-2" onClick={() => navigate('/flashcards')}>
              {isEn ? 'Back to Flashcards' : 'Về Flashcard'}
            </Button>
          </div>
        </div>
      </div>
